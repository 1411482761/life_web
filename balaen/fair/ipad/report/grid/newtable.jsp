...
<head>
	...
	<link type="text/css" href="css/index.css?t=<%=date %>" rel="StyleSheet"/>
	<script type="text/javascript" src="js/jquery.shapeshift.js?t=<%=date %>"></script>
	<script type="text/javascript" src="js/loadgridcontrol.js?t=<%=date %>"></script>
</head>
<body>
	<!--表-->
	<ul class="nav"></ul>
	...
	<div class="container">
	 ...
	 <!--提交按钮-->
	 <div class="submit">
		<button onclick="window.location.replace('rptconfig.jsp?name=<%=name%>')">返回</button><button onclick="lc._savetabledef()">确认</button><button style="display:none;">跳转</button><button style="display:none;">整表删除</button>
	 </div>
</body>

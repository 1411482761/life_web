...
/*
 ...
 if(assocType==RefByTable.ONE_TO_ONE){
	 ...
	 if(objectId !=-1){
	 	if(isInput==true && userWeb.hasObjectPermission(table.getName(),objectId,com.agilecontrol.nea.core.security.Directory.WRITE)){
	 		// write form
	  	...
	  	if( (directoryPerm&com.agilecontrol.nea.core.security.Directory.WRITE)==0){
	...
	request.setAttribute("masterid", String.valueOf(mainTableObjectId));
 	...
 	if(table.getSecurityDirectory().equals(mainTable.getSecurityDirectory())){
 		if( (directoryPerm&com.agilecontrol.nea.core.security.Directory.READ)==0){
	  		// can not read
	  		%>
	 		<font color='red'><%=PortletUtils.getMessage(pageContext, "no-permission",null)%></font>
	 		<%
	  	}else{
	  		if((directoryPerm&com.agilecontrol.nea.core.security.Directory.WRITE)==0 || isInput==false||
	  			"2".equals(request.getAttribute("com.agilecontrol.nea.core.request.object.mainobject.status"))||
	 ...
	 <%
 		...
 		}else if( (directoryPerm&com.agilecontrol.nea.core.security.Directory.READ)!=0 ){
 			request.setAttribute("ismodify", "false");	
 			%>
 			<jsp:include page="<%=NDS_PATH+ \"/object/multiple_object.jsp\"  %>" flush="true" />
 		<%

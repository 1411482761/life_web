...
<head>
	...
	<script type="text/javascript" src="/fair/ipad/grid/js/reportspecialcontrol.js?t=<%=date %>"></script>
</head>
<body>
	<div class="container">
		<!--纵轴-->
		...
		<div class="formula">
			<div class="header">公式定义</div>
			<div class="content">
				<div class="param-a">维度： <span class="coln-a"></span></div>
				<div class="param-b">度量：<span class="coln-b"></span></div>
				<div class="param-c">层级：<span class="coln-c"></span></div>
				<div class="layout">格式显示：<select><option value="0.0%">0.00%</option><option value="$0,0.0">$0.0</option></select></div>
				<div class="define">算法定义：<span>度量/sum(度量)</span></div>
				<div class="showlayout">显示颜色：
					<span class="less" >小于：<span class="coln-less"></span>颜色：<input type="text" name="lesscolor" placeholder="例：#98b77c" onchange="reportspecialcontrol._changeColor('lesscolor')"/><span class="lesscolor"></span></span>
					<span class="greater">大于：<span class="coln-greater"></span>颜色：<input type="text" name="greatercolor" placeholder="例：#ff5534" onchange="reportspecialcontrol._changeColor('greatercolor')"/><span class="greatercolor"></span></span>
					<span class="middle" style="margin-top: 17px;">之间显示颜色：<input type="text" name="middlecolor" placeholder="例：#fed201" onchange="reportspecialcontrol._changeColor('middlecolor')"/><span class="middlecolor"></span></span>
				</div>
				<!--提交按钮-->
				<div class="submit">
					<button onclick="window.location.replace('/fair/ipad/grid/rptconfig.jsp?name=<%=tbname%>')">返回</button><button onclick="reportspecialcontrol._saveRptSpecialColumnDef()">保存</button>
				</div>
			</div>
		</div>
	</div>
	<script type="text/javascript">
		jQuery(document).ready(function(){
			reportspecialcontrol._load(<%=id%>,'<%=cmd%>');
		});
	</script>
</body>
</html>
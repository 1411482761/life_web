...
<%@ page import="com.agilecontrol.nea.core.control.util.*" %>
...
if(userWeb==null || userWeb.isGuest()){
	...
	response.sendRedirect("//control/login?redirect="+redirect);
	return;
}
...
if(!userWeb.hasObjectPermission(table.getName(),objectId,com.agilecontrol.nea.core.security.Directory.READ)){
...
if(table!=null){
	...
	if(objectId !=-1){
			hasWritePermission=(canDelete || canModify ||canSubmit) && 
				userWeb.hasObjectPermission(table.getName(),objectId,com.agilecontrol.nea.core.security.Directory.WRITE);
		 	if(isInput==true && hasWritePermission){
		 		includePage="object_modify.jsp";
		 	}else if(userWeb.hasObjectPermission(table.getName(),objectId,com.agilecontrol.nea.core.security.Directory.READ)){
		 		includePage="object_view.jsp";
		 	}else{
		  	...
		  	// object id==-1
		  	if( (perm&com.agilecontrol.nea.core.security.Directory.WRITE)==0){
...
<div id="obj-top">
<div class="buttons">
	<%= PortletUtils.get(pageContext, "audit-comments",null)%>:<input id="comments" class="form-text" type="text" value="" name="comments" maxlength="255" size="80"/>&nbsp;
...
<%@ include file="inc_command.jsp" %>
	...
	&nbsp;&nbsp;<a href="workflow.jsp?table=<%=tableId%>&id=<%=objectId%>"><%=PortletUtils.getMessage(pageContext, "view-workflow",null)%></a>
	</div>
...
<%
//mark user read, set modifieddate to current time
com.agilecontrol.nea.core.control.util.AuditUtils.markRead(auditid,userWeb.getUserId());
%>
</body>
</html>
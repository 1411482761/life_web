...
ConfigureControl.prototype = {
		...
		recreat: function(name,view){
			...
			data.img='/fair/ipad/img/'+theme+'/report.png';
			data.allow_filter="N";
			data.allow_f_filter="N";
			data.ishide="N";
			data.isacross_report="N";
			element = "<div style='background-image:url(/fair/ipad/img/"+theme+"/report.png)' isnew='Y'><p>新建报表</p><p>"+name+new Date().getTime()+"</p></div>";
		...
		_drawHtml: function(result){
			...
			jQuery(".container").on("click","div",function(){
				...
				if(jQuery(this).hasClass('last')){
					...
					jQuery("#pageurl").val(data.pageurl);
					...
					if(data.referTo.indexOf("view")!=-1 && parseInt(data.referTo.substring(4)) >= 900){
						...
						if(data.isacross_report!=undefined&&data.isacross_report=="Y"){jQuery("#isacross_report").prop("checked",true);}else{jQuery("#isacross_report").prop("checked",false);}
...
//			jQuery(".container").append(element);
			if(!this.isloaded){
				...
				jQuery("span.testrpt").bind("click",function(){
					window.location.href="/fair/ipad/grid/testrptview.jsp";
				});
				//基础报表配置
				jQuery("span.basicrpt").bind("click",function(){
					window.location.href="/fair/ipad/grid/basicrptconfig.jsp";
				});
			}
			cc._draginit(".container",param);
			//报表显示名称
...
//					}
	...
	_detailconfigure : function(){
		...
		var desc = jQuery(".check").children('.reportname').text();
		...
		if(name.indexOf( 'GRPT')==0)
			window.location.href = '/fair/ipad/report/graphicalconfig.jsp?name='+name+"&desc="+desc;
		else
			window.location.replace('/fair/ipad/grid/rptconfig.jsp?name='+name);
	},
	_checkchange : function(){
		...
		if(dom.isacross_report==undefined)dom.isacross_report="N";
	...
	_saverptviews : function(){
		...
		if(jQuery(".isnew").css("display")=="inline"){
			...
			dom.data("data").isacross_report=jQuery("#isacross_report").is(":checked")?"Y":"N";
			dom.data("data").top=jQuery("#topset").val();
			dom.data("data").mergecell=jQuery("#mergecell").val();
			dom.data("data").pageurl=jQuery("#pageurl").val();

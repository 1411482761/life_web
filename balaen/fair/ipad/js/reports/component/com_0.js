									...
									for(var k = 0;k < this.real_calc_column.length;k++){
										if(j == this.real_calc_column[k]){
										  ...
										  for(var s = a1 - flag;s < a1; s++){
											...
											sum = sum/sum_times;
											var temp_sum = sum.toString();
											var len = 0;
											if(temp_sum.toString().indexOf(".") > -1){
												len = temp_sum.substring(temp_sum.toString().indexOf(".")+1,temp_sum.toString().length).length;
											}
											if(len > 2){
												sum = sum.toFixed(2);
											}

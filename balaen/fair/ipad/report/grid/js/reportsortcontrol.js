...
ReportSortControl.prototype = {
		...
		_sortField : function(obj){
			...
			if(type=="asc"){
				dom.attr("src","images/desc.png");
				dom.attr("type","desc");
			}else{
				dom.attr("src","images/asc.png");
				dom.attr("type","asc");
			}
		},
		_addDom : function(result,e){
				...
				for(var i =0; i < result.length; i++){
					if(e=="fields"){
						...
						jQuery(className).append("<div exist='N'><span class='desc'>"+ result[i].description +"</span><span style='display:none;'>"+result[i].dbname+"</span><img alt='' src='images/asc.png' onclick=rsc._sortField(this); type='asc'></div>");
						...
						if(result[i].is_row_orderby!=undefined&&result[i].is_row_orderby=="Y"){
							...
							jQuery(".rowsort").append("<div exist='Y'><span class='desc'>"+ result[i].column_desc +"</span><span style='display:none;'>"+result[i].column_name+"</span><img alt='' src='images/"+type+".png' onclick=rsc._sortField(this); type='"+type+"'></div>");

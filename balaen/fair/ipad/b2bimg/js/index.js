...
Products.prototype = {
			...
			if (response.data[0].code == 0) {
				...
				jQuery(response.data[0].result.products).each(function() {
					jQuery("#list").append('<div class="item">' +
							'<a class="pic" href="javascript:pdts.images(' + this.id + ');"><img src="' + this.imageurl + '?tempid='+Math.random()+'"/></a>' +
							'<input name="checkbox" type="checkbox" value="' + this.id + '" class="checkbox"/>' +
							'<a class="name" href="javascript:pdts.images(' + this.id + ');">[' + this.name + ']</a>' + 
							'<a class="value" href="javascript:pdts.images(' + this.id + ');">' + this.value + '</a>' +
							'</div>');
				});
			} else {
	...
	checkPaipai : function(){
		var req = {
			...
			portalClient.sendOneRequest(req, function(response) {
				if (response.data[0].code == 0) {
					if(response.data[0].result.paipai=="o"){
						pdts.makeSelect();
						pdts.list();
					} else {
	...
	exportOrder: function(table,ids,callback){
 		var trans = [{
			...
			'command': "com.agilecontrol.fair.cmd",
			'params': {
					cmd: "PrintObject",
					table:table,
					format:"xls",

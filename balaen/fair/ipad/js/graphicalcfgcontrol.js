var grc;
var GraficalControl = Class.create();
var flag="N";
GraficalControl.prototype ={
			   ...
			   portalClient.sendOneRequest(trans, function(response){
	            if(response.data[0].code==-1){
                 ...
                 for(var i =0;i<ds.length;i++){
                	 dropdown_menu+="<option value='"+ds[i].name+"'>"+ds[i].desc+"</option>";
                 }
                 ...
                 for(var i=0;i<list.length;i++){
                	 list_group+="<a href='javascript:' onclick='grc.clickChart(this)' class='list-group-item'><div class='row'><img class='col-md-6' src='/fair/ipad/rptgraphical/"+list[i].cptname+"/"+list[i].cptname+".png'></img><div class='col-md-6'><div style='display:none'>"+list[i].chartname+"</div><div class='label label-info'>"+list[i].cptdesc+"</div><div>"+list[i].title+"</div></div></div></a>";
                 }
                 jQuery(".list1").html(list_group);
                 
		    	 var nodes = jQuery('.list1').find(".list-group-item");                
			     if(nodes.length!=0){
			    	 for(var i=0;i<nodes.length;i++){
			    		 jQuery(nodes[i]).children("div").data("params",list[i]);
			    	 }

	                var first = jQuery(".list1").find(".list-group-item").first();
          ...
          loadCptLib: function(){
               ...
               for(var i=0;i<size;i++){
                ...
                html+="<div class='col-md-4' style='cursor:pointer;text-align:center' onclick='grc.clickCpt(this)'><span><img style='margin-top:10px' src='/fair/ipad/rptgraphical/"+javaData[i].name+"/"+javaData[i].name+".png'></span><div style='display:none'>"+javaData[i].name+"</div><div style='text-align:center'>"+javaData[i].desc+"</div></div>";
               }
               html+="</div>";
          ...
          clickCpt:function(event){
        	  ...
        	  jQuery(event).closest("div.row").find("div.selected").removeClass("selected");
        	  ...
        	  this.curCpt={cptname:cptname,cptdesc:cptdesc};
          },
          selectCpt:function(){
        	 var hasProp=false;
          ...
          addChart2List:function(){
        	 ...
        	 var date = new Date();       	  
        	 var chartname = "grc"+date.getFullYear()+grc.checkDate(date.getMonth()+1)+grc.checkDate(date.getDate())
        	                +this.checkDate(date.getHours())+grc.checkDate(date.getMinutes())+grc.checkDate(date.getSeconds());
        	 //get the parameters according to the dom structure
        	 var _params = jQuery("#params").find(".value");
        	 var config=[];
        	 for(var i=0;i<_params.length;i++){
        		 config.push(jQuery(_params[i]).val());
        	 }
        	 var item = {
        			 cptname:cptname,
        			 cptdesc:cptdesc,
        	 ...
        	 var _chartHtml ="<a href='javascript:' onclick='grc.clickChart(this)' class='list-group-item'><div class='row'><img class='col-md-6' src='/fair/ipad/rptgraphical/"+cptname+"/"+cptname+".png'></img><div class='col-md-6'><div style='display:none'>"+chartname+"</div><div class='label label-info'>"+cptdesc+"</div><div>"+title+"</div></div></div></a>";
        	 
        	 jQuery(".list1").append(_chartHtml);
        	 jQuery(".list1").children(".list-group-item").last().children("div").data("params",item);
        	 jQuery(".list1").children(".list-group-item").last().children("div").data("ever_clicked","Y");
          ...
          removeChartFromList:function(){
        	   ...
        	   jQuery(".list1").children(".active").children("div").removeData("params");
          ...
          modifyChart:function(){
        	 ...
        	 jQuery(".list1").children(".active").children("div").data("params",item); 
        	 jQuery(".list1").children(".active").find("img").attr("src","/fair/ipad/rptgraphical/"+cptname+"/"+cptname+".png");
         	 jQuery(".list1").children(".active").find("[style='display:none']").text(chartname);
          ...
          clickChart:function(event){
        	...
        	jQuery(event).closest("a").siblings(".list-group-item").removeClass("active");
            jQuery(event).closest("a").addClass("active");
            
            
  	    	var params = jQuery(".list1").children(".active").children("div").data("params");
  
    	     
      	    if(jQuery(".list1").children(".active").children("div").data("ever_clicked")=="Y"){
      	    	 console.log("clicked");

      	    	var div_id = params.chartname;
      	    	var show = jQuery("#hidden").children("#"+div_id).prop("outerHTML");
      	    	jQuery("#params").html(show);
      	    	
     	        var widgts = jQuery("#params").find(".value");
     	        for(var i=0;i<widgts.length;i++){
     	        	 jQuery(widgts[i]).val(params.config[i]);
     	        }
        	     ...
        	     jQuery("#ds").val(ds);
           	     jQuery("#ds").find("option[value='"+ds+"']").attr("selected",true);
        	  ...
        	  portalClient.sendOneRequest(trans,function(response){
        	     ...
        	     jQuery("#title").val(title);
       		     ...
       		     grc.changeSource(function(){
     		    	   for(var i=0;i<widgts.length;i++){
           		    	   console.log(jQuery(widgts[i]).find("option"));
            ...
            jQuery(".list1").children(".active").children("div").data("ever_clicked","Y");
          ...
          renderParamList :function(params){
        	  ...
        	  var param_html = "";
        	  //add the class -value to identify the dom closest to the value should be stored
        	  for(var i =0;i<d_param.length;i++){
            	  var widgt_type = d_param[i].type;
            	  var temp="";
            	  param_html+="<div class='col-md-"+d_param[i].span+"'><label>"+d_param[i].name+"&nbsp&nbsp</label>";
            	  if(widgt_type=="text"){
            		  temp="<input type='text'  value='"+d_param[i].value+"' class='value' >";
            	  }
            	  if(widgt_type=="droplist"){
            		  var tmp="";
            		  for(var j=0;j<d_param[i].valuelist.length;j++){
            			  tmp+="<option value='"+d_param[i].valuelist[j].value+"'>"+d_param[i].valuelist[j].desc+"</option>";
            		  }
            		  temp ="<select class='btn btn-default value' style='min-width:60%' >"+tmp+"</select>";
         	  ...
         	  var can ="<div id='"+params.chartname+"' class='row'>"+param_html+"</div>";
            		   ...
            		   for(var j=0;j<array.length;j++){
            			   for(var k=0;k<data.length;k++){
            				   if(data[k].type==array[j]){
            					   ...
            					   option+="<option value='"+data[k].value+"'>"+data[k].name+"</option>";
            				   }
            			   }
...
GraficalControl.main = function() {
  portalClient = new PortalClient();
  //设置请求路径
  portalClient.init(null,null,"/servlets/binserv/Request");
  grc = new GraficalControl();
};
jQuery(document).ready(GraficalControl.main);

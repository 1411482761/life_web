...
function caculateByComs(){
	...
	for ( var i = 0; i < coms.length; i++) {
		...
		drawComOne(coms[i],rpt);
	}	
	
	retstr= JSON.stringify(rpt);
}

function drawComOne(com,rpt){
	var c;
	var type = com.type;
	var params = com.params;
	rpt.formula = com.formula;
	rpt.hasImage = "N";
	switch (type) {
	case 0:
		c = Com_0.draw(params,rpt);
		break;
	case 1:
		c = Com_1.draw(params,rpt);
		break;
	case 2:
		c = Com_2.draw(params,rpt);
		break;
	case 3:
		c = Com_3.draw(params,rpt);
		break;
	case 4:
		c = Com_4.draw(params,rpt);
		break;
	case 5:
		c = Com_5.draw(params,rpt);
		break;
	case 6:
		c = Com_6.draw(params,rpt);
		break;
	case 7:
		c = Com_7.draw(params,rpt);
		break;
	default:
		return;
	}
